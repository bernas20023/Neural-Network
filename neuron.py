...
class Network:
  ...
  def backPropagate (self, targets, learning_rate):
    ...
    for k in range(len(targets)):
      if(maxerror<targets[k] - self.matrixOutput[k]):
        maxerror = targets[k] - self.matrixOutput[k]
    return maxerror
...
def run():
  print('Hello World!, This is a neural network that can be used to predict the parity of a binary number.\nOutput will be 1 if the input pattern contains an odd number of 1s and 0 if it contains an even number of 1s.\nWait a moment while the neural network is being trained...')
  ...
  with open('data.csv', 'r') as csvfile:
    ...
    for row in csvreader:
      ...
      trainingData.append([[int(row[0]),int(row[1]),int(row[2]),int(row[3])], [int(row[4])]])
  ...
  #print('Test data:')
  test = input('Insert a binary number with 4 bits:')
  test1 = [int(test[0]),int(test[1]),int(test[2]),int(test[3])]
  nn.test(test1)
